...
function createLSTM(input_size, num_hidden, num_layers,
  ...
  for L = 1, num_layers do
    ...
    if L == 1 then
      ...
      if input_feed then
          x = nn.JoinTable(2)({x, inputs[1+offset]}) -- batch_size x (input_size + num_hidden)
          input_size_L = input_size + num_hidden
      end    
    else 
      x = outputs[(L-1)*2] 
      if dropout then x = nn.Dropout(dropout)(x) end -- apply dropout, if any
      input_size_L = num_hidden
    end
    -- evaluate the input sums at once for efficiency
    local i2h = nn.Linear(input_size_L, 4 * num_hidden)(x)
    local h2h = nn.Linear(num_hidden, 4 * num_hidden)(prev_h)
    local all_input_sums = nn.CAddTable()({i2h, h2h})
    -- decode the gates
    local sigmoid_chunk = nn.Narrow(2, 1, 3 * num_hidden)(all_input_sums)
    sigmoid_chunk = nn.Sigmoid()(sigmoid_chunk)
    local in_gate = nn.Narrow(2, 1, num_hidden)(sigmoid_chunk)
    local forget_gate = nn.Narrow(2, num_hidden + 1, num_hidden)(sigmoid_chunk)
    local out_gate = nn.Narrow(2, 2 * num_hidden + 1, num_hidden)(sigmoid_chunk)
    -- decode the write inputs
    local in_transform = nn.Narrow(2, 3 * num_hidden + 1, num_hidden)(all_input_sums)
  ...
  if use_attention then
    ...
    local decoder_out
    local decoder_attn = create_decoder_attn(num_hidden, 0)
...
function create_decoder_attn(num_hidden, simple)
  ...
  local context_combined = nn.MM()({attn, context}) -- batch_l x source_l x num_hidden
  context_combined = nn.Sum(2)(context_combined) -- batch_l x num_hidden
  local context_output
  if simple == 0 then
    context_combined = nn.JoinTable(2)({context_combined, inputs[1]}) -- batch_l x num_hidden*2
    context_output = nn.Tanh()(nn.LinearNoBias(num_hidden*2, num_hidden)(context_combined))
  else
